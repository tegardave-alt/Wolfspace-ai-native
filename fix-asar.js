...
// Extract from backup
as.extractAll('C:\\Users\\dave\\AppData\\Local\\Programs\\Quantum\\resources\\app.asar.bak3', tmp);
...
m = m.replace(
  ...
  "const emit = (msg) => { try { e.sender.send('quantum:chunk', { id, data: msg }); } catch (_) {} };"
);

// 2. finish: send directly
m = m.replace(
  "const finish = () => { _streams.delete(id); emit({ done: true }); };",
  "const finish = () => { _streams.delete(id); try { e.sender.send('quantum:chunk', { id, data: { done: true } }); } catch (_) {} };"
);
