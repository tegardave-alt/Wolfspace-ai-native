...
function readsStdin(lang, code) {
  ...
  switch (lang) {
    ...
    case 'c':
    case 'cpp':    return /\bscanf\s*\(|\bgets\s*\(|\bgetchar\s*\(|std::cin|\bcin\s*>>/.test(s);
...
function opensGuiWindow(lang, code) {
  ...
  switch (lang) {
    ...
    case 'kotlin':
    case 'java':   return /javax\.swing|java\.awt|javafx\.|JFrame\b|JOptionPane\b|JDialog\b/.test(s);
    case 'javascript': return /require\(\s*['"]electron['"]\s*\)/.test(s);
    case 'c':
    case 'cpp':   return /windows\.h[\s\S]{0,400}CreateWindow|gtk\/gtk\.h|QApplication\b/.test(s);
    default: return false;
  }
}
...
async function runSandboxed(lang, code) {
  ...
  try {
    const { stdout } = await execP('docker ' + args.map(a => /[\s\"]/ .test(a) ? JSON.stringify(a) : a).join(' '), {
...
async function runPy(code) {
  ...
  const PY_BIN = RUN.python || 'python';
  const env = { ...process.env };
  try {
    const { stdout } = await execP(`"${PY_BIN}" "${src}"`, { timeout: EXEC_TIMEOUT, encoding: 'utf8', stdio: ['pipe','pipe','pipe'], env, windowsHide: true });
    return { ok: true, output: stdout };
  } catch (e) {
    return { ok: false, output: (e.stdout || '').toString(), error: procErr(e) };
  } finally {
...
async function runJS(code) {
  ...
  fs.writeFileSync(src, code, 'utf8');
  try {
    const { stdout } = await execP(`"${JS_RUNTIME}" "${src}"`, { timeout: EXEC_TIMEOUT, encoding: 'utf8', stdio: ['pipe','pipe','pipe'], windowsHide: true });
    return { ok: true, output: stdout };
  } catch (e) {
    return { ok: false, output: (e.stdout || '').toString(), error: procErr(e) };
  } finally {
...
async function compileRun(code, ext, compiler, label, env) {
  ...
  try { fs.rmSync(src, { force: true }); fs.rmSync(exe, { force: true }); } catch (_) {}
  return res;
}
...
async function runGo(code) {
  ...
  try {
    ...
    res = { ok: false, output: (e.stdout||'').toString(), error: ((e.stderr||'')+'').trim() || 'error' };
  } finally {
    try { fs.rmSync(dir, { recursive: true, force: true }); } catch (_) {}
  }
  return res;
...
function askModelStream(port, messages, onToken, reg) {
  if (!port || port === '' || Number(port) < 1 || !Number.isFinite(Number(port))) return Promise.reject(new Error('local model tidak aktif — tidak ada port'));
...
const KNOWN = ['python','javascript','typescript','html','css','json','yaml','c','cpp','go','java','kotlin','rust','ruby','php','sql','shell','markdown'];
...
function procErr(e) {
  ...
  if (err) return err;
  if (e.signal) return `proses dihentikan (${e.signal}) — kemungkinan timeout / infinite loop`;
