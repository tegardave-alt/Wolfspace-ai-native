...
function loadCache() {
  try {
    if (fs.existsSync(CACHE_FILE)) {
      ...
      const opt = cfg.prompts && cfg.prompts.self_agent && cfg.prompts.self_agent.metadata && cfg.prompts.self_agent.metadata.optimized;
...
async function optimizeInBackground(originalPrompt) {
  ...
  try {
    ...
    if (result.ok && result.output && result.output.length > 50 && result.output.length < originalPrompt.length * 0.95) {
      ...
      console.log('[sysprompt] DSpy optimization skipped — output not shorter or failed');
    }
  } catch (e) {
