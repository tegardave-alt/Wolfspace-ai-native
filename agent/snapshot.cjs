...
function createSnapshot(filePaths, label = '') {
  ...
  console.log(`[snapshot] Created: ${id} (${savedFiles.length} files) label="${label}"`);
...
function rollback(snapshotId) {
  ...
  console.log(`[snapshot] Rollback: ${snapshotId} → restored ${restored.length} files`);
...
function _pruneOldSnapshots() {
  ...
  snaps.forEach((snap, idx) => {
    ...
    if (tooOld || tooMany) {
      try {
        fs.rmSync(path.join(SNAP_DIR, snap.id), { recursive: true, force: true });
        console.log(`[snapshot] Pruned: ${snap.id} (tooOld=${tooOld}, tooMany=${tooMany})`);
      } catch (_) {}
    }
  });
}

module.exports = { createSnapshot, rollback, listSnapshots };
