...
class MCPClient {
  ...
  async init() {
    ...
    for (const [name, conf] of Object.entries(srvs)) {
      try {
        ...
        dlog('mcp', 'error', `Gagal memulai MCP server ${name}`, { error: e.message });
      }
    }
  ...
  _startServer(name, conf) {
    return new Promise((resolve, reject) => {
      dlog('mcp', 'info', `Memulai server MCP: ${name}`, { cmd: conf.command, args: conf.args });
      
      const env = { ...process.env, ...conf.env };
      const cmd = process.platform === 'win32' && conf.command === 'npx' ? 'npx.cmd' : conf.command;
      ...
      proc.on('error', (err) => {
        dlog('mcp', 'error', `[MCP ${name} process error]`, { err: err.message });
      });
      ...
      this._request(name, "initialize", {
        ...
        dlog('mcp', 'error', `Gagal inisialisasi MCP server ${name}`, { err: err.message });
        reject(err);
      });
    });
  }
  ...
  _handleMessage(name, line) {
    try {
      ...
      if (msg.id !== undefined && !msg.method) {
        ...
        dlog('mcp', 'info', `Menerima request/notif dari ${name}: ${msg.method}`);
      }
    } catch (e) {
      dlog('mcp', 'warn', `Gagal parsing MCP message dari ${name}`, { text: line, err: e.message });
    }
  }
  ...
  async getTools() {
    ...
    for (const name of Object.keys(this.servers)) {
      ...
      try {
        ...
        dlog('mcp', 'error', `Gagal fetch tools dari ${name}`, { error: e.message });
      }
    }
    return allTools;
  }
  ...
  async callTool(prefixedToolName, args) {
    ...
    if (!match) return { ok: false, output: `Nama tool MCP tidak valid: ${prefixedToolName}` };
    ...
    try {
      dlog('mcp', 'info', `Memanggil tool MCP: ${toolName} di ${serverName}`, { args });
      ...
      // Format balasan MCP
      if (res.isError) {
        return { ok: false, output: `[MCP Error] ${(res.content || []).map(c => c.text).join('\\n')}` };
      }
      
      const textOutput = (res.content || []).map(c => c.text).join('\\n');
      return { ok: true, output: textOutput };
    } catch (e) {
      dlog('mcp', 'error', `Gagal memanggil tool ${toolName} di ${serverName}`, { error: e.message });
