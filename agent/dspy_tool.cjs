...
function run(args) {
  ...
  const systemMsg = `You are a prompt optimization expert. Your task is to improve the given prompt following ChainOfThought methodology.
...
<brief explanation of key improvements>`;
  ...
  return new Promise(resolve => {
    try {
      ...
      let full = '';
      askCloudStream(cloud, messages, token => { full += token; }, null)
        .then(() => {
          // Parse optimized prompt from response
          let optimized = full;
          let explanation = '';

          const optMatch = optimized.match(/OPTIMIZED_PROMPT:\s*([\s\S]*?)(?:\nEXPLANATION:|$)/i);
          ...
          if (optMatch) {
            ...
            optimized = lines.slice(0, Math.ceil(lines.length * 0.7)).join('\n').trim();
          }
          if (explMatch) {
        ...
        .catch(err => {
          resolve({ ok: false, output: 'DSpy optimization failed: ' + err.message });
        });
    } catch (e) {
