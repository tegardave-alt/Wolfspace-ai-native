 ...
 */
function chatStream({ history, port, cloud, webdev }, emit, ctl) {
  // Choose system prompt based on history (coding vs normal) – prompts module handles.
  ...
  // Safety: if cloud is specified but has no key, do NOT fall through to local model.
  if (cloud && !cloud.key) {
    const err = new Error('Cloud dipilih tapi API key tidak ditemukan. Simpan API key di menu ☁ dulu.');
    onError(err);
