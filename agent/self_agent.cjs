...
async function selfAgentStream({ history, port, cloud }, emit, ctl = {}) {
  ...
  const messages = [{ role: 'system', content: SELF_FC_SYS }, ...(history || [])];
  const MAX_STEPS = 50;
  let edits = 0;
  const callCounts = {};
  let fallbackCount = 0;
  ...
  try {
    for (let step = 1; step <= MAX_STEPS; step++) {
      ...
      if (!calls || !calls.length) {
        ...
        emit({ t: 'adone', steps: step, edits, summary: finalSummary, backup: backupRel(), run: runReply(fallback) });
        break;
      }
      ...
      for (let i = 0; i < calls.length; i++) {
        ...
        if (results[i] && results[i].stop) {
          ...
          emit({ t: 'adone', steps: step, edits, summary: finalSummary, backup: backupRel(), run: runReply(msg.content || '') });
          return finalSummary;
        }
      }
      for (let i = 0; i < calls.length; i++) {
