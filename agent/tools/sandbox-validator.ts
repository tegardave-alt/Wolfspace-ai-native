...
const { Q_FORBID, qSyntaxOk } = require("./file-tools.ts");
...
const SEMANTIC_CATEGORIES = {
  credential: {
    ...
    confidence: 0.92,
    description:
      "Kredensial/credential — file berisi informasi autentikasi, token, API key, password",
    ...
    // Content-based heuristics — cues inside a file's content (for write and
    // read validation).
    contentHints: [
...
function getPathContext(filePath) {
  ...
  for (const seg of segments) {
    if (
      /^(src|lib|app|components|pages|api|routes|controllers|models|services)$/i.test(
        seg,
      )
    ) {
...
function detectFileIntent(filePath, contentPreview) {
  ...
  for (const [catId, category] of Object.entries(SEMANTIC_CATEGORIES)) {
    ...
    if (
      contentPreview &&
      category.contentHints &&
      category.contentHints.length > 0
    ) {
    ...
    if (matchScore > 0) {
      ...
      const confidence = Math.min(
        1.0,
        category.confidence * (0.5 + matchScore),
      );
      results.push({
 ...
 */
function qSemanticSearch(query: any, options: any = {}) {
  ...
  const KEYWORD_INTENT_MAP = {
    ...
    dist: "build_output",
    distribution: "build_output",
    compiled: "build_output",
    compilation: "build_output",
    config: "config_sensitive",
    configuration: "config_sensitive",
    setting: "config_sensitive",
    settings: "config_sensitive",
    env: "config_sensitive",
    environment: "config_sensitive",
  };
...
function validateBashCommand(cmd) {
  ...
  const destructiveWords =
    /\b(del|delete|remove|rm|rd|format|wipe|clear|purge)\b/i;
  const recursiveFlags = /\b[/-][rRsf]+\b|\/s\b/i;
  const systemPaths =
    /(?:system32|windows|program files|appdata|etc|usr|opt|boot)/i;

  if (
    destructiveWords.test(cmdLower) &&
    recursiveFlags.test(cmd) &&
    systemPaths.test(cmd)
  ) {
    return {
      safe: false,
...
function validateFilePath(filePath, contentPreview) {
  ...
  for (const pattern of FORBIDDEN_FILES) {
    if (pattern.test(normalized)) {
      return {
        safe: false,
        reason: `LEXICAL: file terlarang (${pattern.source}): ${normalized}`,
      };
    }
  }
  ...
  if (!absPath.startsWith(QROOT)) {
    return {
      safe: false,
      reason: `path di luar root WOLFSPACE: ${normalized}`,
    };
  }
...
async function validateEdit(filePath, oldString, newString) {
  ...
  // 3. Check old_string exists (with a smart whitespace-tolerant fallback)
  ...
  try {
    ...
    if (!syntaxCheck.ok) {
      return {
        safe: false,
        reason: `sintaks rusak setelah edit:\n${syntaxCheck.error}`,
      };
    }
  } catch (e) {
    try {
