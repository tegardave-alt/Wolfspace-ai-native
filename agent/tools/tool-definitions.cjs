...
const SELF_TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Execute JS task code with ZERO ambient filesystem/network access, run in a separate process with Node\'s --permission flag (no --allow-fs-read/write grants at all). The code\'s ONLY way to affect the outside world is `await request(capability, params)` -- e.g. `await request("readFile", {path})`, `await request("writeFile", {path, content})`, `await request("fetch", {url})` -- each validated by a deny-by-default policy (scoped to the current workspace dir for files, known cloud-provider hosts for fetch) and logged to an audit trail. Use this instead of sandbox_run when the task needs file access outside its own scratch dir or outbound network calls and that access should actually be enforced, not just requested nicely.',
      parameters: {
        type: "object",
        properties: {
          code: {
