...
const CALL_RE = /<function\s*[:=]\s*([\w.-]+)\s*(?:[:=]\s*(\{[\s\S]*?\}))?\s*\/?>(?:\s*<\/function>)?/g;
...
function parsePseudoCalls(text) {
  ...
  while ((m = CALL_RE.exec(text))) {
    let args = {};
    if (m[2]) {
      try { args = JSON.parse(m[2]); } catch (_) { continue; } // malformed -> drop, don't fabricate
    }
...
// Streaming-safe filter for token-by-token output (agent/chat.cjs's onToken). Buffers only
...
function createPseudoTagStreamFilter(onOutput) {
  ...
  function feed(chunk) {
    ...
    for (;;) {
      const idx = s.indexOf('<');
      if (idx === -1) { if (s) onOutput(s); return; }
      if (idx > 0) { onOutput(s.slice(0, idx)); s = s.slice(idx); }
      if (s.length < NEEDLE.length) {
        if (NEEDLE.startsWith(s)) { hold = s; return; } // could still become "<function"
        onOutput(s[0]);
        s = s.slice(1);
        continue;
      }
      ...
      if (closeTagIdx !== -1) { s = s.slice(closeTagIdx + '</function>'.length); continue; }
      if (selfCloseIdx !== -1 && (plainCloseIdx === -1 || selfCloseIdx <= plainCloseIdx)) {
...
module.exports = { parsePseudoCalls, stripPseudoTags, createPseudoTagStreamFilter };
