...
function _askCloudStreamOnce(cloud, work, onToken, reg) {
  return new Promise((resolve, reject) => {
    ...
    console.log(
      "[cloud] system prompt:",
      sys ? `${sys.length} chars, starts: ${sys.slice(0, 80)}...` : "EMPTY!",
    );
    console.log(
      "[cloud] work messages:",
      workMsgs.length,
      "msgs, roles:",
      workMsgs.map((m) => m.role).join(","),
    );
    let host = cfg.host,
      path = cfg.path,
      port: any = null,
    ...
    const effortTokens =
      cloud.effort === 0 || cloud.effort === "low"
        ? 1024
        : cloud.effort === 2 || cloud.effort === "high"
          ? 16384
          : 4096;
    const effortStr =
      cloud.effort === 0 || cloud.effort === "low"
    ...
    if (cloud.baseUrl) {
      ...
      body = JSON.stringify({
        ...
        generationConfig: { maxOutputTokens: effortTokens + 4096 },
      });
      extract = (j) => {
        try {
    ...
    const t0 = Date.now();
    const maskedKey = cloud.key
      ? cloud.key.slice(0, 5) + "***" + cloud.key.slice(-4)
      : "none";
    dlog("cloud", "info", `cloud model start ${provider}/${model}`, {
      provider,
      model,
      host,
      key: maskedKey,
...
function fillCloudKey(cloud) {
  ...
  if (
    cloud.provider === "opencode" ||
    (cloud.baseUrl && /opencode\.ai/i.test(cloud.baseUrl))
  ) {
    const isFree = cloud.model && cloud.model.includes("-free");
    const correct = "https://opencode.ai" + (isFree ? "/zen/v1" : "/zen/go/v1");
    if (cloud.baseUrl !== correct) {
      cloud.baseUrl = correct;
      cloud.provider = "opencode";
    }
  }
}
