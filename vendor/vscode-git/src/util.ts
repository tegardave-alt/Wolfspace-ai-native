...
// TRIMMED COPY. Upstream util.ts is 872 lines; git.ts imports nineteen names
// from it, and every one of them sits in the first 548 lines. The remainder
// (deltaHistoryItemRefs and after) needs SourceControlHistoryItemRef and
// DiagnosticSeverity from the VS Code API, which is exactly the kind of pull
// that keeps vendor/vscode-terminal off the build path. Cutting there keeps
// this file buildable. Two lines changed inside the slice; both say so.
...
// isRemote removed: it read vscode.env.remoteName, and git.ts does not use it.
