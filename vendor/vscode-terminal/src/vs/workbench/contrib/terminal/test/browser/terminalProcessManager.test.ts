...
suite("Workbench - TerminalProcessManager", () => {
  ...
  setup(async () => {
    ...
    environmentVariableService = instantiationService.get(
  ...
  test("does not accumulate environment variable collection listeners when relaunching", async () => {
    const changeCollectionsEmitter = (
      environmentVariableService as unknown as {
        _onDidChangeCollections: Emitter<unknown>;
      }
    )._onDidChangeCollections;
    const initialListenerCount = listenerCount(changeCollectionsEmitter);

    await manager.createProcess({}, 80, 24, false);
    strictEqual(
      listenerCount(changeCollectionsEmitter),
