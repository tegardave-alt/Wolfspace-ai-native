...
suite("DecorationAddon", () => {
  ...
  suite("registerDecoration", () => {
    ...
    test("should dispose decoration resources when the decoration is disposed", () => {
      const marker = xterm.registerMarker(2)!;
      const decoration = decorationAddon.registerCommandDecoration({
        command: "cd src",
        marker,
        exitCode: 0,
        timestamp: Date.now(),
      ...
      const decorations = (
        decorationAddon as unknown as { _decorations: Map<number, unknown> }
      )._decorations;

      decoration.dispose();

      strictEqual(hoverDisposed, true);
      deepStrictEqual(removedEventListeners.sort(), [
        "click",
        "contextmenu",
        "mousedown",
      ]);
