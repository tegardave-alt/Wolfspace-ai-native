...
class TestTerminalInstance extends mock<ITerminalInstance>() {
  ...
  private readonly _onDisposed = this.store.add(
...
class TestTerminalService extends mock<ITerminalService>() {
  private readonly _ptyFactories: NonNullable<
    IShellLaunchConfig["customPtyImplementation"]
  >[] = [];
    ...
    return {
      object: {
        value: {
          title: "Test Terminal",
          content: [],
          claim: { kind: TerminalClaimKind.Client, clientId: this.clientId },
        },
...
suite("AgentHostTerminalService", () => {
  ...
  test("instance disposal locally disposes a created PTY without deleting the host terminal", async () => {
    ...
    await pty.start();
    assert.strictEqual(
      connection.disposedSubscriptions,
      0,
      "the subscription should be live while the instance is live",
    );
    ...
    const reconnectResult = await service.reconnectTerminals(
      connection,
      connection.clientId,
    );
    ...
    assert.deepStrictEqual(
      ...
      },
      {
        ...
        dispatchedActions: [
          { type: ActionType.TerminalResized, cols: 80, rows: 30 },
        ],
  ...
  test("instance disposal locally disposes a revived attach-only PTY and allows revival again", async () => {
    ...
    const instance = await service.reviveTerminal(
      connection,
      terminalUri,
      "tool-session",
    );
    ...
    instance.dispose();
    instance.dispose();
    const replacement = await service.reviveTerminal(
      connection,
      terminalUri,
      "tool-session",
    );
  ...
  test("a late revived PTY cannot replace the current PTY registration", async () => {
    const terminalUri = URI.parse("agenthost-terminal:/tool-terminal");
    const oldInstance = await service.reviveTerminal(
      connection,
      terminalUri,
      "tool-session",
    );
