...
suite("Workbench - ChatTerminalCommandMirror", () => {
      ...
      deepStrictEqual(
        {
          ...
          missingCharWidth: computeChatTerminalMirrorCols(
            1224,
            makeFont(undefined),
            1,
          ),
  ...
  suite("DetachedTerminalSnapshotMirror.layout", () => {
    ...
    function createSnapshotMirror(
      output:
        { text: string; truncated?: boolean; lineCount?: number } | undefined,
    ): DetachedTerminalSnapshotMirror {
      return store.add(
        instantiationService.createInstance(
          DetachedTerminalSnapshotMirror,
      ...
      deepStrictEqual(
        {
          ...
          maxColumnWidth: computeMaxBufferColumnWidth(
            fakes[0].raw.buffer.active,
            fakes[0].raw.cols,
          ),
        },
        {
  ...
  suite("DetachedTerminalCommandMirror.layout", () => {
     ...
     */
    async function createWrappedCommand(
      source: XtermTerminal,
    ): Promise<ITerminalCommand> {
  ...
  suite("row height metrics", () => {
    ...
    setup(async () => {
      ...
      instantiationService.stub(ITerminalService, {
        createDetachedTerminal: async (options: IDetachedXTermOptions) => {
          const fake = createFakeDetachedTerminal(
            XTermBaseCtor,
            options,
            nextFont,
          );
    ...
    test("snapshot mirror onDidChangeRowHeight fires once per metrics change, only after attach", async () => {
      ...
      strictEqual(
        fires,
        0,
        "rendering content alone must not fire before attach",
      );
    ...
    test("command mirror onDidChangeRowHeight fires once per metrics change, only after attach", async () => {
      ...
      strictEqual(
        fires,
        1,
        "the first render after attach announces the metrics",
      );
