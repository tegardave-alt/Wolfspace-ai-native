  ...
  implements Partial<ITerminalInstanceService>
{
  ...
  constructor() {
    super();
    this._processCreatedPromise = new Promise(
      (resolve) => (this._resolveProcessCreated = resolve),
    );
  }
...
suite("Workbench - TerminalInstance", () => {
  ...
  suite("TerminalInstance", () => {
    ...
    function createTerminalInstantiationService(
      ...
      instantiationService.stub(
        ITerminalInstanceService,
        terminalInstanceService ?? store.add(new TestTerminalInstanceService()),
      );
      ...
      instantiationService.stub(
        ...
        workspaceTrustRequestService ??
          ({ requestWorkspaceTrust } as Partial<IWorkspaceTrustRequestService>),
      );
    ...
    test("marked remote resolver terminal bypasses workspace trust request", async () => {
      const workspaceTrustRequestService =
        new TestTerminalWorkspaceTrustRequestService();
      ...
      await (instance as unknown as Record<string, () => Promise<void>>)[
        "_createProcess"
      ]();
      ...
      deepStrictEqual(
        {
          ...
          persistedResolverFlag:
            instance.shellLaunchConfig[remoteResolverTerminal],
        },
        {
          trustRequestCount: 0,
    ...
    test("should not create a process when workspace trust is denied", async () => {
      const terminalInstanceService = store.add(
        new TestTerminalInstanceService(),
      );
      let resolveTrust!: (trusted: boolean) => void;
      const trustRequest = new Promise<boolean>(
        (resolve) => (resolveTrust = resolve),
      );
      const instantiationService = createTerminalInstantiationService(
        terminalInstanceService,
        undefined,
    ...
    test("should not create a process with an unexpected cwd in an empty workspace", async () => {
      ...
      const createProcess = () =>
        (instance as unknown as Record<string, () => Promise<void>>)[
          "_createProcess"
        ]();
      ...
      ok(
        exitResult &&
          ...
          typeof exitResult.message === "string",
      );
    ...
    test("should fire onWillDispose before xterm disposal and onDisposed after xterm disposal", async () => {
      ...
      store.add(
        instance.onWillDispose(() => disposalOrder.push("onWillDispose")),
      );
    ...
    test("should dispose contribution-owned xterm addons before xterm disposal", async () => {
      ...
      deepStrictEqual(
        ...
        {
          disposalOrder: ["onWillDispose", "addon", "xterm", "onDisposed"],
          addonDisposeCount: 1,
        },
      );
    });
  ...
  suite("TerminalLabelComputer", () => {
    ...
    test("should resolve cwd when under userHome", () => {
      ...
      terminalLabelComputer.refreshLabel(
        createInstance({ capabilities, cwd: "/home/user/foo/bar" }),
      );
    ...
    test("should resolve cwd when exactly at userHome", () => {
      ...
      terminalLabelComputer.refreshLabel(
        createInstance({ capabilities, cwd: "/home/user" }),
      );
      strictEqual(terminalLabelComputer.title, "~");
      strictEqual(terminalLabelComputer.description, "~");
    });
    ...
    test("should use ${sequence} for Command Code agent CLI shell type", () => {
      ...
      terminalLabelComputer.refreshLabel(
        createInstance({
          capabilities,
          shellType: GeneralShellType.CommandCode,
          sequence: "Fix Parser Bug",
          processName: "node",
        }),
      );
  ...
  suite("getCwdResource", () => {
    ...
    > {
      ...
      // Mock file service
      mockFileService = {
        canHandleResource: async (_resource: URI) =>
          options.fileServiceCanHandle !== false,
