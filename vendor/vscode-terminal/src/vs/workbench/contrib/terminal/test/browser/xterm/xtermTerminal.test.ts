...
suite("XtermTerminal", () => {
  ...
  setup(async () => {
    ...
    const lifecycleService = instantiationService.get(
      ILifecycleService,
    ) as TestLifecycleService;
    onWillShutdown = (
      lifecycleService as unknown as { _onWillShutdown: Emitter<unknown> }
    )._onWillShutdown;
    ...
    deepStrictEqual(
      {
        regularXtermListeners:
          listenerCountAfterRegularXterm - lifecycleListenerCountBeforeXterm,
        detachedXtermListeners:
          listenerCount(onWillShutdown) - listenerCountAfterRegularXterm,
      },
      {
  ...
  test("disables custom glyphs when moved into an auxiliary window", async () => {
    ...
    store.add(
      toDisposable(() => (auxiliaryDocument.createElement = createElement)),
    );
