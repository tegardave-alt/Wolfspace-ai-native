...
suite("Workbench - TerminalService", () => {
  ...
  suite("background terminals", () => {
    test("should remove disposed hidden terminals and their listeners", async () => {
      const disposalEmitters = Array.from({ length: 3 }, () =>
        store.add(new Emitter<ITerminalInstance>()),
      );
      const instances = disposalEmitters.map(
        (emitter, index) =>
          ({
            instanceId: index + 1,
      ...
      const backgroundedTerminalDisposables = Reflect.get(
        terminalService,
        "_backgroundedTerminalDisposables",
      ) as { size: number };
