...
class MockAgentConnection implements IAgentConnection {
  ...
  readonly onMcpNotification: Event<
    import("../../../../../platform/agentHost/common/agentService.js").IMcpNotification
  > = Event.None;
  readonly initializeResult: IObservable<
    | import("../../../../../platform/agentHost/common/state/protocol/common/commands.js").InitializeResult
    | undefined
  ...
  async invokeChangesetOperation(): Promise<{}> {
    return {};
  }
  async handleMcpRequest(): Promise<unknown> {
  ...
  async getManagedSettingsDiagnostics(): Promise<
  ...
  async getSessionStateFile(_session: URI): Promise<URI | undefined> {
    throw new Error("Not implemented");
  }
  async collectDebugLogs(
    _session: URI | undefined,
    _kind: AgentHostDebugLogsArtifactKind,
  ): Promise<IAgentHostDebugLogsArtifact> {
    throw new Error("Not implemented");
  }
  async readDebugLogsChunk(
    _resource: URI,
    _position: number,
  ): Promise<IAgentHostDebugLogsChunk> {
    throw new Error("Not implemented");
  }
  ...
  ): Promise<ListAutomationTriggerDefinitionsResult> {
    return { items: [] };
  }
  async runAutomation(
  ...
  ): Promise<FetchAutomationRunsResult> {
    return {};
  }
  ...
  getSubscription<T>(
    ...
    const listener = this._onDidAction.event((envelope) => {
      if (envelope.channel === _resource.toString()) {
        ...
        this._terminalState = terminalReducer(
          this._terminalState,
          envelope.action as TerminalAction,
        );
...
suite("AgentHostPty", () => {
  ...
  test("start() creates terminal and subscribes", async () => {
    ...
    const pty = disposables.add(
      new AgentHostPty(1, conn, terminalUri, { name: "test" }, logService),
    );
  ...
  test("start() fires onProcessReady", async () => {
    ...
    disposables.add(conn);
    const pty = disposables.add(
      new AgentHostPty(1, conn, terminalUri, undefined, logService),
    );
  ...
  test("output channel follows accumulated state without creating a pty", () => {
    const conn = new MockAgentConnection({
      isPty: false,
      content: [{ type: "unclassified", value: "existing\n" }],
    });
    disposables.add(conn);
    const source = disposables.add(
      new AgentHostOutputChannel(conn, terminalUri),
    );
  ...
  test("terminal/exited action finalizes the local PTY exactly once", async () => {
    const conn = new MockAgentConnection();
    disposables.add(conn);
    const pty = new TestAgentHostPty(
      1,
      conn,
      terminalUri,
      undefined,
  ...
  test("shutdown() disposes terminal and unsubscribes", async () => {
    ...
    assert.strictEqual(
      exitFired,
      false,
      "shutdown should not emit exit synchronously",
    );
    assert.deepStrictEqual(
      conn.disposedTerminals.map((uri) => uri.toString()),
      [terminalUri.toString()],
      "shutdown should dispose the host terminal synchronously",
    );
  ...
  test("shutdown() retries host disposal after pending terminal creation settles", async () => {
    ...
    assert.deepStrictEqual(
      conn.disposedTerminals.map((uri) => uri.toString()),
      [terminalUri.toString(), terminalUri.toString()],
    );
  });
  ...
  test("shutdown() finalizes locally when host disposal throws synchronously", async () => {
    ...
    assert.deepStrictEqual(
      ...
      },
      {
        disposeCount: 1,
        warnings: [
          "[AgentHostPty] Failed to dispose host terminal: client unavailable",
        ],
      },
    );
  });
  ...
  test("reconnect() re-subscribes with new connection and replays content", async () => {
    ...
    const pty = disposables.add(
      new AgentHostPty(1, conn1, terminalUri, undefined, logService),
    );
  ...
  test("reconnect() during pending terminal creation supersedes start()", async () => {
    ...
    const pty = disposables.add(
      new TestAgentHostPty(1, conn1, terminalUri, undefined, logService),
    );
    let readyCount = 0;
    disposables.add(pty.onProcessReady!(() => readyCount++));
    const dataReceived: string[] = [];
    ...
    const conn2 = new MockAgentConnection({
      content: [{ type: "unclassified", value: "recovered output\n" }],
    });
    disposables.add(conn2);
  ...
  test("a stale hydration timeout does not affect a successfully reconnected PTY", () =>
    runWithFakedTimers({}, async () => {
      const conn1 = new MockAgentConnection();
      disposables.add(conn1);
      const warnings: string[] = [];
      const pty = disposables.add(
        new TestAgentHostPty(
          1,
          conn1,
          terminalUri,
          undefined,
      ...
      const exitCodes: (number | undefined)[] = [];
      disposables.add(
        pty.onProcessExit!((exitCode) => exitCodes.push(exitCode)),
      );
      const dataReceived: string[] = [];
      disposables.add(
        pty.onProcessData!((e) =>
      ...
      const conn2 = new MockAgentConnection();
      disposables.add(conn2);
      const hydration: { state: TerminalState | undefined } = {
        state: undefined,
      };
      ...
      onDidApplyAction.fire({
        channel: terminalUri.toString(),
        action: { type: ActionType.TerminalData, data: "post-timeout data" },
        serverSeq: 1,
        origin: undefined,
      });
  ...
  test("shutdown() cancels pending reconnect hydration", async () => {
    ...
    const dataReceived: string[] = [];
    disposables.add(
      pty.onProcessData!((event) =>
        dataReceived.push(typeof event === "string" ? event : event.data),
      ),
    );
    await pty.start();
    ...
    onDidChange.fire({
      title: "Late",
      content: [{ type: "unclassified", value: "late data" }],
      claim: { kind: TerminalClaimKind.Client, clientId: "test-client" },
      lifecycle: { status: TerminalLifecycleStatus.Running },
    });
    onDidApplyAction.fire({
      channel: terminalUri.toString(),
      action: { type: ActionType.TerminalData, data: "late action" },
      serverSeq: 1,
      origin: undefined,
    });
    await Promise.resolve();
  ...
  test("reconnect() times out when subscription never hydrates", () =>
    runWithFakedTimers({}, async () => {
      const conn1 = new MockAgentConnection();
      disposables.add(conn1);
      const pty = disposables.add(
