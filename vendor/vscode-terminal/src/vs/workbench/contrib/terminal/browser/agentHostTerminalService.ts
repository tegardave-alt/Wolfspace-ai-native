  ...
  implements IAgentHostTerminalService
{
  ...
  >();
  private readonly _pendingRevives = new Map<
    string,
    Promise<ITerminalInstance>
  >();
  ...
  async createTerminal(
    ...
    const ptyRegistration = this._createPtyRegistration(
      key,
      connection.clientId,
    );
  ...
  async reviveTerminal(
    ...
    const revive = this._doReviveTerminal(
      connection,
      terminalUri,
  ...
  attachOutputTerminal(
    ...
    const source = store.add(
      new AgentHostOutputChannel(connection, terminalUri),
    );
    store.add(
      this._terminalChatService.registerOutputSource(
        terminalToolSessionId,
        source,
      ),
    );
  ...
  private async _doReviveTerminal(
    ...
    const instancePromise = Promise.resolve().then(() =>
  ...
  private _createPtyRegistration(
    key: string,
    clientId: string,
  ): IAgentHostPtyRegistration {
   ...
   */
  private _registerInstancePtyCleanup(
    instance: ITerminalInstance,
    key: string,
    ptyRegistration: IAgentHostPtyRegistration,
  ): void {
    const cleanup = () => {
