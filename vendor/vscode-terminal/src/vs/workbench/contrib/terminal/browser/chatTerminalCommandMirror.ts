...
interface IDetachedTerminalCommandMirror {
  ...
  layout(
    widthPx: number,
  ): Promise<IDetachedTerminalCommandMirrorRenderResult | undefined>;
...
export function computeChatTerminalMirrorCols(
  ...
  horizontalChromePx: number = ChatTerminalMirrorMetrics.MirrorHorizontalPaddingPx,
): number {
  if (!isFinite(availableWidthPx) || availableWidthPx <= 0 || !font.charWidth) {
    return ChatTerminalMirrorMetrics.MirrorColCountFallback;
  }
  const dpr =
    isFinite(devicePixelRatio) && devicePixelRatio > 0 ? devicePixelRatio : 1;
...
function getMirrorRaw(detached: IDetachedTerminalInstance): RawXtermTerminal {
  return (detached.xterm as IDetachedXtermTerminal & { raw: RawXtermTerminal })
    .raw;
}
 ...
 */
function getMirrorDevicePixelRatio(
 ...
 */
function measureMirrorHorizontalChrome(
  detached: IDetachedTerminalInstance,
): number | undefined {
 ...
 */
function getMirrorRowHeightPx(
  detached: IDetachedTerminalInstance | undefined,
): number | undefined {
 ...
 */
export function computeSnapshotLineCount(
  buffer: {
  ...
  const hasCursorLineContent = !!buffer
    .getLine(cursorLineIndex)
  ...
  implements IDetachedTerminalCommandMirror
{
  ...
  private _detachedTerminalPromise:
    Promise<IDetachedTerminalInstance> | undefined;
  ...
  private readonly _onDidChangeRowHeightEmitter = this._register(
    new Emitter<void>(),
  );
  public readonly onDidChangeRowHeight: Event<void> =
    this._onDidChangeRowHeightEmitter.event;
  ...
  private _installFirstRenderListener(
    detached: IDetachedTerminalInstance,
  ): void {
    ...
    this._register(
      getMirrorRaw(detached).onRender(() => this._notifyRowHeightIfChanged()),
    );
  }
  ...
  async layout(
    ...
    const cols = computeChatTerminalMirrorCols(
      widthPx,
      detached.xterm.getFont(),
      getMirrorDevicePixelRatio(detached),
      measureMirrorHorizontalChrome(detached),
    );
...
export class DetachedTerminalSnapshotMirror extends Disposable {
  ...
  constructor(
    ...
    private readonly _getTheme: () =>
      IChatTerminalToolInvocationData["terminalTheme"] | undefined,
    @ITerminalService private readonly _terminalService: ITerminalService,
    @IContextKeyService private readonly _contextKeyService: IContextKeyService,
  ) {
    super();
  ...
  public async attach(container: HTMLElement): Promise<void> {
    ...
    if (!this._renderListenerInstalled) {
      ...
      // changes are announced, so the per-frame cost is one number comparison.
      this._register(
        getMirrorRaw(terminal).onRender(() => {
          const rowHeight = this.getRowHeightPx();
          if (
            rowHeight !== undefined &&
            rowHeight !== this._lastObservedRowHeight
          ) {
            this._lastObservedRowHeight = rowHeight;
            this._onDidChangeRowHeightEmitter.fire();
          }
  ...
  public async layout(
    widthPx: number,
  ): Promise<{ lineCount?: number; maxColumnWidth?: number } | undefined> {
    if (widthPx <= 0) {
      return undefined;
    }
    return this._renderSequencer.queue(async () => {
      ...
      const cols = computeChatTerminalMirrorCols(
        widthPx,
        terminal.xterm.getFont(),
        getMirrorDevicePixelRatio(terminal),
        measureMirrorHorizontalChrome(terminal),
      );
      ...
      const lineCount = computeSnapshotLineCount(
        terminal.xterm.buffer.active,
        this._output?.truncated ? this._output.lineCount : undefined,
      );
      this._lastRenderedLineCount = lineCount;
      if (this._shouldComputeMaxColumnWidth(lineCount)) {
        this._lastRenderedMaxColumnWidth =
          this._computeMaxColumnWidth(terminal);
      }
      return { lineCount, maxColumnWidth: this._lastRenderedMaxColumnWidth };
    });
  }

  private async _render(): Promise<
    { lineCount?: number; maxColumnWidth?: number } | undefined
  > {
    ...
    if (!text) {
      if (this._lastRenderedText) {
        await new Promise<void>((resolve) =>
          terminal.xterm.write("\x1b[2J\x1b[3J\x1b[H", resolve),
        );
      }
    ...
    if (write) {
      await new Promise<void>((resolve) =>
        terminal.xterm.write(write, resolve),
      );
    }
    if (this._store.isDisposed) {
      return undefined;
    ...
    const lineCount = computeSnapshotLineCount(
      terminal.xterm.buffer.active,
      output.truncated ? output.lineCount : undefined,
    );
    this._renderedVersion = outputVersion;
