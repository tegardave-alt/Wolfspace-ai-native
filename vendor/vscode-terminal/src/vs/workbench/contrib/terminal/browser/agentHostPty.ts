...
export class AgentHostPty extends BasePty implements ITerminalChildProcess {
  ...
  private readonly _subscription = this._register(
  ...
  > {
    ...
    try {
      ...
      if (!this._options?.attachOnly) {
        ...
        const created = await raceCancellation(
          terminalCreation.then(() => true),
      ...
      if (
        this._lifetime.token.isCancellationRequested ||
      ...
      const { subscription, store } = this._createSubscription(
        this._connection,
      );
  ...
  private _createSubscription(connection: IAgentConnection): {
    readonly subscription: IAgentSubscription<TerminalState>;
  ...
  private async _waitForHydration(
    subscription: IAgentSubscription<TerminalState>,
    ...
    const timeout =
      timeoutMs === undefined
        ? undefined
        : disposableTimeout(
            () => hydration.error(new Error("Reconnect hydration timed out")),
            timeoutMs,
            store,
          );
    store.add(toDisposable(() => hydration.complete(false)));
    store.add(
      Event.once(subscription.onDidChange)(() => {
  ...
  private _disposeHostTerminal(): void {
    try {
      void this._connection
        .disposeTerminal(this._terminalUri)
        .catch((err) => this._logHostDisposalError(err));
    } catch (err) {
  ...
  private _logHostDisposalError(err: unknown): void {
    this._logService.warn(
      `[AgentHostPty] Failed to dispose host terminal: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
  ...
  async reconnect(newConnection: IAgentConnection): Promise<boolean> {
    ...
    try {
      ...
      if (
        this._lifetime.token.isCancellationRequested ||
        this._subscription.value !== subscriptionStore
      ) {
        return false;
      }
      ...
      if (!this._lifetime.token.isCancellationRequested) {
        this._logService.warn(
          `[AgentHostPty] Reconnection failed: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
