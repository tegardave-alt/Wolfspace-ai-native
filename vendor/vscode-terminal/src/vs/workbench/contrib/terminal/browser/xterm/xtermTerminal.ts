  ...
  implements IXtermTerminal, IDetachedXtermTerminal, IInternalXtermTerminal
{
  ...
  private readonly _webglContextLossListener = this._register(
  ...
  *getBufferReverseIterator(): IterableIterator<string> {
  ...
  getContentsAsText(
    ...
    const startLine =
      startMarker === undefined || startMarker.line === -1
        ? 0
        : startMarker.line;
   ...
   */
  private _getScrollbarOptions():
    { width: number; overviewRuler: { showTopBorder: boolean } } | undefined {
  ...
  private async _enableWebglRenderer(): Promise<void> {
    ...
    if (
      (this._webglAddon || this._webglAddonLoading) &&
  ...
  private _getWebglCustomGlyphs(): boolean {
    ...
    return (
      ...
      this.raw.element?.ownerDocument === this._mainDocument
    );
  }
  ...
  private async _refreshLigaturesAddon(): Promise<void> {
    ...
    if (ligaturesConfig?.enabled) {
      ...
      if (
        ...
        !equals(ligatureOptions, this._ligaturesAddonConfig)
      ) {
