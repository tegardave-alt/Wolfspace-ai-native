...
export class TerminalService extends Disposable implements ITerminalService {
  ...
  private readonly _backgroundedTerminalDisposables = this._register(
  ...
  async createTerminal(
    ...
    if (shellLaunchConfig.hideFromUser) {
      ...
      this._backgroundedTerminalDisposables.set(
        instance.instanceId,
        instance.onDisposed((instance) =>
          this._onBackgroundTerminalDisposed(instance),
        ),
      );
      this._onDidChangeInstances.fire();
      return instance;
  ...
  private _onBackgroundTerminalDisposed(instance: ITerminalInstance): void {
    const index = this._backgroundedTerminalInstances.findIndex(
      (backgrounded) => backgrounded.instance === instance,
    );
    if (index !== -1) {
