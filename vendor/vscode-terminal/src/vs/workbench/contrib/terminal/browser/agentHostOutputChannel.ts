 ...
 */
export class AgentHostOutputChannel
  extends Disposable
  implements IChatTerminalOutputSource
{
  ...
  constructor(connection: IAgentConnection, terminalUri: URI) {
    super();
    const subscriptionRef = this._register(
    ...
    this._register(
      subscription.onDidChange((state) => this._acceptState(state)),
    );
  }
