  ...
  implements ICommandDetectionCapability
{
  ...
  private _nextCommandId:
    { command: string; commandId: string | undefined } | undefined;
  ...
  get executingCommandConfidence(): "low" | "medium" | "high" | undefined {
    const casted = this._currentCommand as
      PartialTerminalCommand | ITerminalCommand;
  ...
  constructor(
    ...
    this._register(
      this._promptInputModel.onDidInterrupt(
        () => (this._isCurrentCommandInterrupted = true),
      ),
    );
      ...
      this.onCommandExecuted((command) => {
        if (command.commandLineConfidence !== "high") {
          ...
          const typedCommand = command as
            ITerminalCommand | PartialTerminalCommand;
