...
describe("ikon per bahasa di pohon berkas", () => {
  ...
  test("SVG asli dari material-icon-theme, DI-VENDOR bukan jadi dependensi", () => {
    ...
    expect(IKON).toMatch(
      // Bertipe sejak berkasnya jadi .ts — pola ini menerima keduanya, karena
      // yang dijaga adalah ADANYA tabel, bukan anotasinya.
      /const IKON_BAHASA(: Record<string, string>)? = \{/,
    );
    expect(IKON).toMatch(/GENERATED BY scripts\/ikon-bahasa\/build\.cjs/);
  ...
  test("bahasa yang lazim dipakai punya ikonnya", () => {
    ...
    // Anotasi tipe dilepas dulu: berkasnya TypeScript sekarang, dan
    // `: Record<string, string>` bukan JavaScript yang sah — Function()
    // melemparkan SyntaxError sebelum sempat menyentuh satu ikon pun.
    const IKON_JS = IKON.replace(": Record<string, string> =", " =");
    const tabel = new Function(IKON_JS + "; return IKON_BAHASA;")();
  ...
  test("dimuat SEBELUM app.tsx memakainya", () => {
    ...
    expect(html).toContain('"/app/IkonBahasa.ts"');
  });
