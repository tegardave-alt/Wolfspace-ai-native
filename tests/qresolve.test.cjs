...
describe('qResolve', () => {
  ...
  it('melempar Error "path di luar root" untuk path di luar QROOT', () => {
    expect(() => qResolve('../../etc/passwd', false)).toThrow(/path di luar root/);
  });
  ...
  it('melempar Error "path terlarang" untuk node_modules/foo.cjs', () => {
    expect(() => qResolve('node_modules/foo.cjs', false)).toThrow(/path terlarang/);
  });
  ...
  it('melempar Error "path tidak boleh ditulis" untuk package.json dengan mustBeEditable=true', () => {
    expect(() => qResolve('package.json', true)).toThrow(/path tidak boleh ditulis/);
  });
  ...
  // 7. Path public/app.jsx dengan mustBeEditable=true harus return path absolut
  it('mengembalikan path absolut untuk public/app.jsx dengan mustBeEditable=true', () => {
    const result = qResolve('public/app.jsx', true);
    expect(path.isAbsolute(result)).toBe(true);
    expect(result).toBe(path.resolve(QROOT, 'public/app.jsx'));
  });
