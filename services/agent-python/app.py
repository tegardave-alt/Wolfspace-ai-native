...
def _make_nodes(run: Run):
    ...
    def tools(state: AgentState) -> dict[str, Any]:
        ...
        need_approve: list[dict[str, Any]] = []
        ...
        for call in last.get("tool_calls") or []:
            ...
            if result.get("needs_approval"):
                need_approve.append({"name": name, "args": call.get("args") or {}})
        ...
        if need_approve:
            ...
            update["pendingToolCalls"] = need_approve
            update["finalSummary"] = (
                f"{len(need_approve)} perintah perlu persetujuan Anda sebelum dijalankan."
            )
        return update
