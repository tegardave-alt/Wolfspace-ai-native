...
const trace = require('../core/agent/trace.cjs');
let _evSeq = 0;
...
const { fillCloudKey } = require('../core/agent/cloud.cjs');
...
function qResolve(p, mustBeEditable) {
  ...
  if (mustBeEditable && !Q_ALLOWED.test(relNorm.replace(/\//g, path.sep)) && !Q_ALLOWED.test(relNorm))
    throw new Error('path tidak boleh ditulis (sumber kode di web/, studio/lib/, atau *.cjs): ' + relNorm);
  return dest;
}
function qList() {
...
//   **/ Ã¢â€ â€™ zero-or-more directories (so web/**/*.jsx matches web/app.jsx)
...
async function qSyntaxOk(absPath) {
  ...
  try {
    ...
    if (ext === '.jsx') {
      const B = require(path.join(QROOT, 'web', 'vendor', 'babel.min.js'));
...
const SELF_SYS = [
  ...
  'ALWAYS wrap each action in a fenced ```block``` (```READ web/app.jsx```). Do not write actions as loose prose. Keep prose minimal.',
  'For a plain question, just answer in DONE Ã¢â‚¬â€ no tools needed.',
  'You may READ/GREP/GLOB anything in the project. You may EDIT/WRITE source files under: server.cjs, any *.cjs at root, config.json, web/** (.jsx/.js/.css/.html/.json), studio/lib/**/*.dart, studio/pubspec.yaml, studio/web/index.html. Forbidden: cloud-keys.json, node_modules, build outputs, backups.',
...
const SELF_TOOLS = [
  ...
  { type: 'function', function: { name: 'glob', description: 'Find files by wildcard (e.g. web/**/*.jsx, *agent*).', parameters: { type: 'object', properties: { pattern: { type: 'string' } }, required: ['pattern'] } } },
...
// Tolerant fallback: weaker models write the action as plain text ("READ web/app.jsx")
...
- The result MUST compile. No prose, no markdown fences. Only the hunks.`;
  ...
  if (req.method === 'POST' && urlPath === '/api/revert-hunk') {
    ...
    req.on('end', async () => {
      try {
        ...
        const { rejectHunk } = require('../core/agent/tools.cjs');
  ...
  if (req.method === 'POST' && urlPath === '/api/apply-hunk') {
    ...
    req.on('end', async () => {
      try {
        const { hunkId } = JSON.parse(body);
        if (!hunkId) { res.writeHead(400, { 'Content-Type': 'application/json' }); return res.end(JSON.stringify({ error: 'hunkId wajib' })); }
        const { applyHunk } = require('../core/agent/tools.cjs');
  ...
  if (req.method === 'GET' && urlPath !== '/') {
    const pubDir = path.join(__dirname, '..', 'web');
  ...
  fs.readFile(HTML, (e, data) => {
    if (e) { res.writeHead(404); return res.end('web/index.html not found'); }
