...
function Composer({
  ...
  const loadMcpServers = React.useCallback(async () => {
    ...
    try {
      ...
      // OPT-IN DIAGNOSTIC (localStorage.wolfspace_mcp_debug = "1"): if a
      // connected server ever vanishes from the list, this shows exactly what
      // /mcp returned and what the row set became, on the platform where it
      // happens. Off by default; costs nothing.
      try {
        if (localStorage.getItem("wolfspace_mcp_debug"))
          console.log(
            "[mcp-dbg] composer load — config ids:",
            Object.keys(data),
            "status ids:",
            Object.keys(st),
          );
  ...
  // ── Command palette: MCP ──
  // Only "Add Server" is exposed. The per-server Connect/Disconnect entries were
  // removed on request — connection management lives in the composer's MCP menu,
  // not the palette.
  usePerintah(
    () => [
      {
