...
contextBridge.exposeInMainWorld('quantum', {
  ...
  terminal: {
    open: (opts) => ipcRenderer.invoke('quantum:invoke', { channel: 'terminal', payload: { action: 'open', cwd: opts?.cwd, shell: opts?.shell } }),
    write: (id, data) => ipcRenderer.invoke('quantum:invoke', { channel: 'terminal', payload: { action: 'write', id, data } }),
    read: (id, clear) => ipcRenderer.invoke('quantum:invoke', { channel: 'terminal', payload: { action: 'read', id, clear } }),
    resize: (id, cols, rows) => ipcRenderer.invoke('quantum:invoke', { channel: 'terminal', payload: { action: 'resize', id, cols, rows } }),
    close: (id) => ipcRenderer.invoke('quantum:invoke', { channel: 'terminal', payload: { action: 'close', id } }),
    list: () => ipcRenderer.invoke('quantum:invoke', { channel: 'terminal', payload: { action: 'list' } }),
  },
});
