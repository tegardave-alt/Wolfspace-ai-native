...
/**
 * Choose the appropriate system prompt based on the chat history and mode.
 * @param {Array} work - chat history
 * @param {boolean} [webdev] - if true, use WEBDEV_SYS (A2UI mode)
 * @returns {string} system prompt
 */
function pickSystem(work, webdev) {
  if (webdev) return WEBDEV_SYS;
