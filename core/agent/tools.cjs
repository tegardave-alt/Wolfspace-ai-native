...
function diskWalk(dir, filterRe, maxDepth) {
  const skip =
    /^(node_modules|\.git|_agent_backups|dist-app|build|\.dart_tool|vendor|__pycache__|\.cache|\.vs|\.nuget|packages|Debug|Release|obj|bin|\.next|\.nuxt|target|bower_components|\.terraform|vendor|cache)$/i;
  const secret =
...
const SELF_TOOLS = [
  ...
  },
  {
    ...
    function: {
      name: "web_search",
      description:
        "Search the web via StackOverflow+GitHub+npm+Wikipedia+DDG. Returns top results with title/URL/snippet.",
      parameters: {
        type: "object",
  ...
  },
  {
    ...
    function: {
      name: "disk_list",
      description:
        'List contents of ANY directory on the local disk. Use absolute paths like "C:\\Users\\dave\\project". Returns files and folders with sizes.',
      parameters: {
        type: "object",
        properties: {
          path: {
            type: "string",
            description:
              'absolute directory path (e.g. "C:\\Users\\dave\\Downloads")',
          },
        },
        required: ["path"],
      },
  ...
  },
  {
    ...
    function: {
      name: "disk_read",
      description:
        "Read ANY file on the local disk by absolute path. Supports near=<line> for context around a line number.",
      parameters: {
        type: "object",
        properties: {
          path: {
            type: "string",
            description: "absolute file path on local disk",
          },
          near: {
            type: "number",
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          pattern: {
            type: "string",
            description: 'glob pattern (e.g. "*.py", "**/*.tsx")',
          },
        },
  ...
  },
  {
    ...
    function: {
      name: "git",
      description:
        "Git version control: diff, status, add, commit, log, branch, revert. Pass git_op + optional file/msg/action/name/limit.",
      parameters: {
        type: "object",
        properties: {
          git_op: {
  ...
  },
  {
    ...
    function: {
      name: "edit_batch",
      description:
        "Atomically edit multiple files. If any fails, ALL rollback. Pass edits array: [{path, old_string, new_string}, ...]",
      parameters: {
        type: "object",
  ...
  },
  {
    ...
    function: {
      name: "run_tests",
      description:
        "Run project tests. Auto-detects npm/python/dart/etc. Optional filter for specific test files.",
      parameters: {
        type: "object",
        properties: { filter: { type: "string" }, cwd: { type: "string" } },
        required: [],
      },
    },
  },
  {
    ...
    function: {
      name: "opencode",
      description:
        "Delegate a task to OpenCode CLI (external AI coding agent). Runs opencode run <prompt> non-interactively. Use for tasks needing a second opinion or external project context.",
      parameters: {
        type: "object",
        properties: {
          prompt: {
            type: "string",
            description: "task/prompt to send to OpenCode CLI",
          },
          model: {
            type: "string",
            description:
              "model name (e.g. opencode/deepseek-v4-pro). Default: opencode/deepseek-v4-flash-free",
          },
          cwd: {
            type: "string",
            description:
              "working directory (absolute path). Default: user home",
          },
        },
        required: ["prompt"],
      },
  ...
  try {
    if (cmd === "diff") {
      ...
      const out = require("child_process")
        .execSync(c, { cwd: QROOT, encoding: "utf8", maxBuffer: 1e6 })
        .trim();
      return { ok: true, output: out || "(no changes)" };
    }
    if (cmd === "status") {
      const out = require("child_process")
        .execSync("git status --short", { cwd: QROOT, encoding: "utf8" })
        .trim();
    ...
    if (cmd === "log") {
      const f = file ? path.relative(QROOT, file) : null;
      const c = f
        ? `git log --oneline -${limit} "${f}"`
        : `git log --oneline -${limit}`;
      const out = require("child_process")
        .execSync(c, { cwd: QROOT, encoding: "utf8" })
        .trim();
      return { ok: true, output: out };
    }
    if (cmd === "branch") {
      if (args.action === "list") {
        const out = require("child_process")
          .execSync("git branch -a", { cwd: QROOT, encoding: "utf8" })
          .trim();
        return { ok: true, output: out };
      }
      if (args.action === "create" && name) {
        require("child_process").execSync(`git branch "${name}"`, {
          cwd: QROOT,
        });
        return { ok: true, output: `created branch: ${name}` };
      }
      if (args.action === "delete" && name) {
        require("child_process").execSync(`git branch -D "${name}"`, {
          cwd: QROOT,
        });
    ...
    if (cmd === "revert" && file) {
      const f = path.relative(QROOT, file);
      require("child_process").execSync(`git checkout -- "${f}"`, {
        cwd: QROOT,
      });
...
function runTests(cwd, filter) {
  try {
    ...
    if (!cmd)
      return {
        ok: false,
        output: "no test framework detected (npm/python/dart/maven/gradle)",
      };
    const out = require("child_process")
      .execSync(cmd, {
        cwd,
...
function runSelfTool(name, args, emit) {
  ...
  try {
    if (name === "list") {
      ...
      trace.emit("tool_exec_done", "tools", {
        tool: name,
        ms: Date.now() - t0,
        ok: true,
      });
    ...
    if (name === "opencode") {
      ...
      if (!prompt) return { ok: false, output: "prompt wajib diisi" };
      const model = args.model || "opencode/deepseek-v4-flash-free";
      const cwd = args.cwd || os.homedir();
      return new Promise((resolve) => {
        const child = spawn("opencode", ["run", prompt, "--model", model], {
          cwd,
          windowsHide: true,
          timeout: 120000,
          env: { ...process.env },
        ...
        child.on("close", (code) => {
          ...
          if (timedOut)
            return resolve({
              ok: false,
              output: "TIMEOUT (120s): OpenCode tidak merespons",
            });
          const out = (stdout || stderr || "").trim();
          resolve({
            ok: true,
            output: out.slice(0, 8000) || "(OpenCode: no output)",
          });
        });
        child.on("error", (err) => {
          clearTimeout(timer);
          resolve({
            ...
            output:
              ...
              err.message +
              ". Pastikan opencode terinstall (npm i -g @opencode-ai/sdk).",
          });
        });
      });
    }
    ...
    if (name === "edit_batch") {
      ...
      for (const b of backups) {
        if (!b.orig.includes(b.old_string))
          return {
            ok: false,
            output: `ABORT: old_string not found in ${b.path}; no changes applied.`,
          };
      }
      for (const b of backups) {
      ...
      return {
        ok: true,
        edited: true,
        output: `edited ${backups.length} file(s) atomically`,
      };
    }
    if (name === "run_tests") {
    ...
    trace.emit("tool_exec_error", "tools", {
      tool: name,
      ms: Date.now() - t0,
