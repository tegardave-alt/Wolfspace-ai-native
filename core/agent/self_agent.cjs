...
const SELF_FC_SYS = [
  ...
  "PLAN phase (default when task starts): Use ONLY read tools (list, glob, read, grep, web_search, web_fetch, bash, disk_*, git diff/status/log). NO edits or writes. Understand the codebase and design your approach.",
  "BUILD phase: After planning, write BUILD_START in your response to transition. Then ALL tools are available (edit, write, edit_batch, run).",
  "IMPORTANT: Plan FIRST before making any changes. Do NOT skip planning even for simple tasks — always read relevant files first.",
  "",
  ...
  "DISK EXPLORATION: Use disk_list, disk_read, disk_glob, disk_grep to explore ANY directory on the user's local disk (not just Quantum's source). The user home is '" +
    os.homedir() +
    "'. Use absolute paths like 'C:\\Users\\dave\\project'. These are READ-ONLY — to edit/write files outside Quantum, use the bash tool with a cwd parameter. The bash tool also supports a cwd parameter to run commands in any directory.",
  ...
  "EXTERNAL AGENTS: Use `opencode` tool to delegate a task to OpenCode CLI (external AI agent). It runs \`opencode run <prompt>\` non-interactively and returns output. Use when user asks to 'pakai opencode' or delegate to another agent for a second opinion or external project work.",
...
async function selfAgentStream({ history, port, cloud }, emit, ctl = {}) {
  ...
  try {
    emit({
      ...
      cloud: {
        ...
        baseUrl: cloud && cloud.baseUrl,
        hasKey: !!(cloud && cloud.key),
      },
    });
  } catch (e) {}
  trace.emit("config", "self-agent", {
    runId,
    provider: cloud && cloud.provider,
    hasKey: !!(cloud && cloud.key),
  });
  ...
  const isLocalBase =
    cloud && cloud.baseUrl && /(127\.0\.0\.1|localhost)/.test(cloud.baseUrl);
  ...
  let backup = null;
  const backupRel = () =>
    backup ? require("path").relative(__dirname, backup) : null;
  const ensureBackup = () => {
    if (!backup) {
      backup = qBackup();
  ...
  if (agentState) {
    ...
    emit({
      t: "dbg",
      m: "self-agent:resumed, step " + (agentState.meta?.steps || "?"),
    });
    if (!agentState.phase) agentState.phase = "plan"; // ensure phase for old checkpoints
  } else {
    agentState = initState({ runId, history, cloud });
    agentState.messages.unshift({ role: "system", content: SELF_FC_SYS });
    agentState.phase = "plan"; // start in PLAN phase
    emit({ t: "phase", p: "plan" });
  }

  const messages = agentState.messages;
  const MAX_STEPS = ctl.maxSteps ?? 9999;
  let edits = 0;
  const callCounts = agentState.callCounts || {};
  let fallbackCount = 0;
  const _TRANSIENT_SELF =
    /ECONNRESET|ETIMEDOUT|EPIPE|socket hang up|timeout|EAI_AGAIN|network|ECONNREFUSED|ENOTFOUND|503|too busy|Service Unavailable|service_unavailable/i;

  try {
    for (let step = 1; step <= MAX_STEPS; step++) {
      if (isCancelled()) {
        dlog("self", "info", "stop", { reason: "cancelled", step });
        break;
      }
      emit({ t: "step", n: step });
      let msg;
      try {
        try {
          emit({
            t: "dbg",
            m: "self-agent:about-to-call-askCloudTools",
            step,
            messages: messages.length,
            tools: Array.isArray(SELF_TOOLS) ? SELF_TOOLS.length : null,
          });
        } catch (e) {}
        msg = await askCloudTools(
          cloud,
          messages,
          vscodeAdapter.patchConfig({ tools: SELF_TOOLS }).tools,
        );
      } catch (e) {
        ...
        trace.emit("error", "self-agent", {
          runId,
          msg: ((e && e.message) || String(e)).slice(0, 200),
        });
        dlog("self", "info", "stop", {
      ...
      if (isCancelled()) {
        dlog("self", "info", "stop", { reason: "cancelled_after_tools", step });
        break;
      }
      messages.push(msg);

      // Check for BUILD_START to transition from PLAN to BUILD phase
      if (
        agentState.phase === "plan" &&
        msg.content &&
        msg.content.includes("BUILD_START")
      ) {
        agentState.phase = "build";
        emit({ t: "phase", p: "build" });
        dlog("self", "info", "transition", { from: "plan", to: "build", step });
      }
      ...
      const runOne = async (tc) => {
        trace.emit("tool_call_start", "self-agent", {
          runId,
        ...
        if (
          agentState.phase === "plan" &&
          /^(edit|write|edit_batch)$/i.test(tc.function.name)
        ) {
          return {
            out: "EDIT/WRITE/EDIT_BATCH diblokir di fase PLAN. Selesaikan perencanaan (analisis, baca, cari info). Ketik BUILD_START untuk beralih ke fase Build.",
          };
        }
        ...
        if (callCounts[sig] >= 2)
          out +=
            "\n[catatan: panggilan sama diulang " +
            callCounts[sig] +
            "× — hasilnya SAMA. JANGAN ulang yang sama; pakai hasil ini, lalu lanjut atau jawab.]";
        ...
        if (tc.function.name === "task") {
          ...
          try {
            const ret = await selfAgentStream(
              { history: [{ role: "user", content: args.goal || "" }], cloud },
              subEmit,
        ...
        trace.emit("tool_call_done", "self-agent", {
          runId,
          tool: tc.function.name,
          ms: Date.now() - t0,
          ok: !!r.ok,
        ...
        if (results[i] && results[i].stop) {
          if (results[i].waitForAnswer) {
            ...
            messages.push({
              ...
              content: results[i].out,
            });
            emit({
              t: "adone",
              steps: step,
              edits,
              summary: "Menunggu jawaban user: " + results[i].question,
              backup: backupRel(),
              waitForAnswer: true,
            });
            dlog("self", "info", "stop", {
          ...
          dlog("self", "info", "stop", {
            ...
            step,
            tool: calls[i].function.name,
          });
          finalSummary =
            msg.content || "Berhenti: panggilan tool berulang tanpa kemajuan.";
      ...
      for (let i = 0; i < calls.length; i++) {
        messages.push({
          role: "tool",
          tool_call_id: calls[i].id,
          content: (results[i] && results[i].out) || "(ok)",
        });
      }
      // Checkpoint after each step
      agentState = reduceState(agentState, {
        messages,
        callCounts,
        meta: { ...agentState.meta, steps: step },
      });
      try {
        checkpoint.save(agentState);
      } catch (_) {}
